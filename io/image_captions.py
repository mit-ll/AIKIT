# slightly modified - https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.image_captions.ImageCaptionLoader.html
import os
...
class ImageCaptionLoader(BaseLoader):
    ...
    def __init__(
        ...
        blip_processor: str = "Salesforce/blip-image-captioning-large",
        ...
        local_path = os.path.join('./hub/',blip_model)

        if os.path.exists(local_path):
            self.processor = BlipProcessor.from_pretrained(local_path)
            self.model = BlipForConditionalGeneration.from_pretrained(local_path)
        else:
    ...
    def _get_captions_and_metadata(
        self, model: Any, processor: Any, image: Union[str, Path, bytes], max_new_tokens=10240
    ) -> Tuple[str, dict]:
